...
func (bc *BlockChain) HandleBlock(b *block.Block) { // block refers to the script block.go 
	// TODO: Implement this function
	if bc.CoinDB.ValidateBlock(b.Transactions){
		bc.CoinDB.StoreBlock(b.Transactions) // store in CoinDB, which is no the actual disk 

		undo_blocks := bc.makeUndoBlock(b.Transactions)
		block_record := bc.ChainWriter.StoreBlock(b, undo_blocks, bc.Length+1)  // create block records

		bc.BlockInfoDB.StoreBlockRecord(b.Hash(), block_record) // store in the actual disk 
